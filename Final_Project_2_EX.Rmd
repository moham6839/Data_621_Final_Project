...
# Expermentiation and Results

After our EDA, we chose following 2 statistical methods to determine which model is best suited for our data: 

- Logistic Regression
- Random Forest

For our logistic regression model, we went for the binary logistic regression model. After the first attempt, we improved the model further with StepAIC.

For our random forest model, 

At the end, we decided that the random forest model is better fitted since it have a higher AUC and below ROC Curve, compared to the Binary Logistic Regression.
